...
|---|---|---|---|
| **Shell** | `main` → `site/` | `deploy-site.yml` on every push to `main` | `index.html`, `assets/`, `hub.config.json`, `docs/` |
| **Content** | `gh-pages` only | the devhub action, from any repo's CI | `reports/`, `data/manifest.json`, `data/runs.json`, `data/badges/` |

The deploy workflow copies the shell into `gh-pages` without touching published content,
and publishing never touches the shell — so both can run at any time.
