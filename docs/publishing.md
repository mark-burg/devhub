...
on top of the newer branch and retried. Publishing only adds content; if the hub's site has
never been deployed, the action warns you to run *Deploy hub site* once.
...
[`site/public/hub.config.json`](../hub.config.json). Files you reference live in `site/public/`
on `main` and are deployed by the *Deploy hub site* workflow.
...
```bash
npm install
npm run demo      # seed .devhub-preview/ with a week of demo runs (real Allure 3 reports via npx)
npm run dev       # http://localhost:5173, hot reload
npm run preview   # production build served like gh-pages, http://localhost:4173
```

Or publish a real local report: `node scripts/hub.mjs publish --site .devhub-preview --project my-app --report e2e --source allure-report`.
